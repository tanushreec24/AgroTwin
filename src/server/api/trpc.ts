...
/**
 * Reusable middleware that enforces users are logged in via TRPC Context.
...
const timingMiddleware = t.middleware(async ({ path, next }) => {
  ...
  result.ok
    ? console.log("✅", path, "took", durationMs, "ms")
    : console.log("❌", path, "took", durationMs, "ms");
  return result;
});
...
export const protectedProcedure = t.procedure
  ...
  .use(({ ctx, next }) => {
    // Since we removed authentication, this is now just a public procedure with timing
    return next({
      ctx,
    });
  });
